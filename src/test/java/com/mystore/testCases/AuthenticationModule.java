...
import com.mystore.utilities.Input;
...
public class AuthenticationModule extends BaseClass {
    ...
    public void createAccountUsingValidInfo() throws IOException, InterruptedException {
        ...
        accountCreationPage.submitAccountCreationForm(Input.title, Input.firstName, Input.lastName, email, Input.password, Input.birthDay, Input.birthMonth, Input.birthYear, Input.addressFirstName, Input.addressLastName, Input.company, Input.address, Input.address2, Input.city, Input.state, Input.postalCode, Input.country, Input.additionalInfo, Input.homePhone, Input.mobilePhone, Input.ref);
        Thread.sleep(5000);
    ...
    public void signInUsingValidData() throws IOException, InterruptedException {
        authPage = new AuthPage(driver);
        authPage.submitSignInForm(Input.email, Input.pass);
        Thread.sleep(3000);
    ...
    public void resetPasswordUsingValidData() throws IOException, InterruptedException {
        authPage = new AuthPage(driver);
        authPage.submitForgetPassEmailForm(Input.email);
        Thread.sleep(3000);

        if (driver.getPageSource().contains("A confirmation email has been sent to your address: " + Input.email)) {
            logger.info("Test Passed");
            Assert.assertTrue(true);
        } else {
            logger.warn("Test Failed");
